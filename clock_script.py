...
def minute_change(device):
    ...
    def helper(current_y):
        with canvas(device) as draw:
            text(draw, (0, 1), hours, fill="white", font=proportional(TINY_FONT))
            text(draw, (15, 1), ":", fill="white", font=proportional(TINY_FONT))
            text(draw, (17, current_y), minutes, fill="white", font=proportional(TINY_FONT))
        time.sleep(0.1)
...
def main():
    ...
    while True:
        ...
        if sec == 59:
            ...
            show_message(device, full_msg, fill="white", font=proportional(TINY_FONT))
            animation(device, 8, 1)
        else:
            ...
            minutes = datetime.now().strftime('%M')
            with canvas(device) as draw:
                text(draw, (5, 1), hours, fill="white", font=proportional(TINY_FONT))
		...
		print(randrange(10))
		rmin = randrange(1,59)
		text(draw, (1,9), str(rmin), fill="white", font=proportional(TINY_FONT))
            time.sleep(0.5)
